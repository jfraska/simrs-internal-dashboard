...
const TableLayoutDetailGudang = ({
  ...
  const handleContinueConfirmationDelete = () => {
    btnDeleteHandler(confirmationDelete);
    handleCloseConfirmationDelete();
  };

  const checkboxChange = (payload) => {
    if (isCheckbox === "single") {
      setSelectedRow([payload]);
      checkboxHandler(payload);
    } else {
      let tempData = [...selectedRow];
      if (tempData.includes(payload)) {
        tempData = tempData.filter((value) => value !== payload);
      } else {
        tempData.push(payload);
      }
      setSelectedRow(tempData);
      checkboxHandler(tempData);
    }
  };
      ...
      <div className={st.container}>
        ...
        >
          {isBtnAdd && (
            ...
            >
              {customBtnAddTitle ? <>{customBtnAddTitle}</> : <>{title}</>}
            </Button>
          )}
        </div>
                                  ...
                                  >
                                    <Checkbox
                                      checked={
                                        isCheckbox === "single"
                                          ? selectedRow[0] === idR
                                          : selectedRow.includes(idR)
                                      }
                                      onChange={(e) => checkboxChange(idR)}
                                    />
                                  </TableCell>
                                )}
...
export default TableLayoutDetailGudang;
