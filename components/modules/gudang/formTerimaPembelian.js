...
import { createPembelian, updatePembelian } from "api/gudang/pembelian";
...
import DialogAddItem from "./dialogAddItemTerimaPembelian";
...
const FormTerimaPembelian = ({
  ...
  const createPembelianValidation = useFormik({
    ...
    onSubmit: async (values, { resetForm, setFieldError }) => {
      let messageContext = isEditType ? "diterima" : "ditambahkan";
      let data = { ...values };
      ...
      try {
        ...
        setSnackbar({
          state: true,
          type: "error",
          message: `Terjadi kesalahan, "${response.data.data.nomor_faktur}" gagal ${messageContext}!`,
        });
      }
    },
  });
  ...
  const updateDetailDataHandler = (payload) => {
    let tempData = [...createPembelianValidation.values.gudang];
              ...
              {isEditType ? (
                <LoadingButton
                  ...
                  disabled={
                    ...
                    !isActionPermitted("receive:store")
                  }
                  startIcon={<DoneIcon />}
...
export default FormTerimaPembelian;
