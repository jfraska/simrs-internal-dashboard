...
const TableLayout = ({
  ...
  const [jenisSuratRef, setJenisSuratRef] = useState({
    values: {
      id: "",
      name: "",
    },
  ...
  const handleOnChangeJenisGudang = (value) => {
    if (value) {
      setJenisGudangRef((e) => ({
        ...e,
        values: {
  ...
  const handleOnChangeJenisSurat = (value) => {
    if (value) {
      setJenisSuratRef((e) => ({
        ...e,
        values: {
  ...
  const handleOnChangeSupplier = (value) => {
    if (value) {
      setSupplierRef((e) => ({
        ...e,
        values: {
  ...
  const handleChangeDate = (newValue) => {
    ...
    tempFilterDisplay = tempFilterDisplay.filter((e) => e.type !== filter);
    tempFilterDisplay.unshift({
      type: tempOptions[0].label.toLowerCase().replace(/\s+/g, "_"),
      label: tempOptions[0].label,
                          ...
                          .map((row, idR) => {
                            return (
                              <TableRow
                                ...
                                key={row.id || idR}
                                onDoubleClick={
                                  row.id && isPermitted("show") && isShow
                                    ? () =>
                                        router.push(
